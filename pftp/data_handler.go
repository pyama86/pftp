...
func (d *dataHandler) Close() error {
	...
	if d.clientConn.conn != nil {
		err = d.clientConn.conn.Close()
	}
	if d.originConn.conn != nil {
		err = d.originConn.conn.Close()
	}

	// close listener
	if d.clientConn.listener != nil {
		err = d.clientConn.listener.Close()
	}
	if d.originConn.listener != nil {
		err = d.originConn.listener.Close()
	}
...
func (d *dataHandler) StartDataTransfer() error {
	...
	defer func() {
		if err := d.Close(); err != nil {
			d.log.debug("got error at close data connections")
		} else {
